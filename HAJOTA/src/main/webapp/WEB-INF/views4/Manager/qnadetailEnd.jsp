...
<script type="text/javascript">
	...
	// 답변하기버튼을 누르면 [1]이 나오게끔 하자
	 function goReply() {
		
		var changeFrm = document.changeFrm;
		changeFrm.action = "/hajota/answerEnterEndManager.go";
...
<div style="background-color: none; width:100%;">
	<span style="font-size: 25px; font-weight: bold;">세부 Q&A 전체 페이지</span>  
</div>
<br/>
...
<div class="container" style="background-color: none; width:70%; height:100%; margin-left:0%;">  
    ...
    <div style="margin-left : 0%;">     
			...
			<div class="form-group" style="text-align: center; margin-left:-30%; margin-top:-10%">
			    ...
			    <input type="text" style="margin-left:28%; width: 30%;" name="seq_question" id="seq_question" value="${n.SEQ_QUESTION}" class="form-control" readonly/>		    			  		    
	  		</div>
			...
			<div style="margin-left: 80%; margin-top:-50%"> 
			    ...
			    <c:if test="${m == null}">	    
				    ...
				    <div style="margin-left:110%; margin-top: 5%">
					    	<button type="button" class="btn btn-danger" onClick="goReply();">답변하기</button>
					</div>
				</c:if>
				
				<c:if test="${m != null}">	    
					<textarea rows="10" cols="80" style="margin-left:10%; margin-top:10%; width: 190%; height: 50%" id="reply" name="reply" class="form-control" readonly="readonly">${m.CONTENT}</textarea>
					</div>
				</c:if>
			</c:if>
	</div> 
